...
use SigRH\Entity\FonteSeguro as FonteSeguroEntity;

class FonteSeguro extends AbstractRepository {

    public function getQuery($search = array()) {
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select('f')
                ->from(FonteSeguroEntity::class, 'f')
                ->orderby('f.descricao','ASC');
        
        if ( !empty($search['search'])){
            $qb->where('f.descricao like :busca');
