...
use SigRH\Entity\Endereco as EnderecoEntity;
...
class Endereco extends AbstractRepository {
    ...
    public function getQuery($search = array()) {
        ...
        $qb->select('e')
                ->from(EnderecoEntity::class, 'e')
                ->orderby('e.endereco','ASC');
        
        if ( !empty($search['search'])){
            $qb->where('e.endereco like :busca');
    ...
    public function getListParaCombo(){
        ...
        $list = $this->findAll();
        foreach($list  as $row){
            $array[] = array("id"=>$row->id,"nome"=>$row->endereco);
        }
        return $array;
    }
