...
use SigRH\Entity\LinhaOnibus as LinhaOnibusEntity;

class LinhaOnibus extends AbstractRepository {

    public function getQuery($search = array()) {
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select('l')
                ->from(LinhaOnibusEntity::class, 'l')
                ->orderby('l.descricao','ASC');
        
        if ( !empty($search['search'])){
            $qb->where('l.descricao like :busca');
