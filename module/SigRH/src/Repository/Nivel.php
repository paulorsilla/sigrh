...
use SigRH\Entity\Nivel as NivelEntity;

class Nivel extends AbstractRepository {

    public function getQuery($search = array()) {
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select('n')
                ->from(NivelEntity::class, 'n')
                ->orderby('n.descricao','ASC');
        
        if ( !empty($search['search'])){
            $qb->where('n.descricao like :busca');
